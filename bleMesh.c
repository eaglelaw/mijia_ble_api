...
uint16 bleMesh_ProcessEvent( uint8 task_id, uint16 events )
{
    ...
    if ( events & BLEMESH_START_DEVICE_EVT )
    {
        ...
        appl_mesh_sample();
        {
          ...
          LOG("++++++++++UUID+++++++++++\n");
          for(i = 0; i< 16; i++)
            LOG("0x%.2x, ", UI_lprov_device.uuid[i]);
          LOG("\n==========UUID+++++++++++\n");
        }
...
//            else if(LL_STATUS_ERROR_UNEXPECTED_STATE_ROLE == pMsg->hdr.status)
    ...
    case GAP_LINK_ESTABLISHED_EVENT:
    {
		...
		param.connect.type = pPkt->devAddrType;
		param.connect.conn_param.min_conn_interval = GAP_GetParamValue(TGAP_CONN_EST_INT_MIN);
		param.connect.conn_param.max_conn_interval = GAP_GetParamValue(TGAP_CONN_EST_INT_MAX);
		param.connect.conn_param.slave_latency = GAP_GetParamValue(TGAP_CONN_EST_LATENCY);
		param.connect.conn_param.conn_sup_timeout = GAP_GetParamValue(TGAP_CONN_EST_SUPERV_TIMEOUT);
    ...
    case GAP_LINK_TERMINATED_EVENT:
    {
		...
		param.disconnect.reason = pPkt->reason;
